...
############Please comment on the below lines of code first and calibrate the initial position of the gripper (Place a block with an ArUco marker on it and let the gripper hold the block)########################
    xyt = pd.DataFrame({'X':cam2aruco_x,'Y':cam2aruco_y,'T':cam2aruco_t})
    thetas = model.predict(xyt)
    theta_6 = pd.DataFrame(thetas[0])
    theta_5 = pd.DataFrame(thetas[1])
    theta_4 = pd.DataFrame(thetas[2])
    theta_3 = pd.DataFrame(thetas[3])
